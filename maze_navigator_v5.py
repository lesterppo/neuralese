"""
Neuralese Remote Brain v5 — PPO + Diversity Loss Hybrid
Combines PPO Actor-Critic (low-variance credit assignment) with diversity loss
(forces per-step latent variation) to achieve both high success rate AND emergence.

Key insight from v4 failure: without diversity loss, the Observer converges to a
static "go right and down" encoding that works ~17% of the time but never varies.
PPO alone can't produce emergent communication — you need explicit incentive for
the latents to CHANGE across steps.
"""
import torch
...
GAMMA = 0.99
LR = 1e-3
RL_LR = 1e-3           # Higher LR to get gradients flowing
ENTROPY_COEF = 0.005
VALUE_COEF = 0.5
DIVERSITY_WEIGHT = 0.15  # Initial diversity weight (will decay to 0.01)
DIVERSITY_DECAY = 0.8   # Multiplicative decay per epoch
WALL_PENALTY = -10.0
GOAL_REWARD = 10.0
STEP_PENALTY = -0.05
TARGET_THRESH = 0.5
...
class ObserverActorCritic(nn.Module):
    """MLP Observer: shared trunk → actor (12D Neuralese) + critic (scalar value)."""
    def __init__(self):
        super().__init__()
    ...
    def forward(self, grid_flat, pos):
        ...
        v = self.critic_head(shared_feat.detach())  # Detach critic to prevent bullying
        return z, v
...
class Navigator(nn.Module):
    """3×3 radar + 12D Neuralese → Normal distribution over movement."""
    def __init__(self):
        super().__init__()
        self.shared = nn.Sequential(
    ...
    for _ in range(batch_size):
        ...
        if path and len(path) >= 2:
            for i in range(len(path) - 1):
                ...
                all_items.append((grid.flatten(), pos, radar, move))
    return all_items
...
# --- PPO with Diversity Loss ---

def collect_trajectories_diverse(observer, navigator, num_eps):
    """Collect rollouts WITH diversity loss — track latent vectors for per-step variation penalty."""
    ...
    for _ in range(num_eps):
        ...
        pos = start.clone()
        active = True
        ep_grids, ep_pos, ep_radars, ep_actions = [], [], [], []
        ep_log_probs, ep_rewards, ep_values, ep_terminated, ep_zs = [], [], [], [], []
...
def compute_gae(rewards_list, values_list, terminated_list, observer, all_obs_grids, all_obs_pos):
    ...
    for i in range(len(rewards_list)):
        rewards, values = rewards_list[i], values_list[i]
        terminated, grids, positions = terminated_list[i], all_obs_grids[i], all_obs_pos[i]
        T = len(rewards)
        ...
        for t in reversed(range(T)):
            mask = 0.0 if terminated[t] else 1.0
            delta = rewards[t] + GAMMA * next_value * mask - values[t]
            gae = delta + GAMMA * GAE_LAMBDA * mask * gae
            advantages[t] = gae
            returns[t] = gae + values[t]
            next_value = 0.0 if terminated[t] else values[t]
        all_advantages.append(advantages)
        all_returns.append(returns)
    return all_advantages, all_returns

def compute_diversity_loss(all_zs):
    """Penalize static latents: reward per-step variation in ||z_t - z_{t-1}||."""
    total_drift = 0.0
    n_pairs = 0
    for ep_zs in all_zs:
        if len(ep_zs) < 2:
            continue
        for t in range(1, len(ep_zs)):
            total_drift += torch.norm(ep_zs[t] - ep_zs[t-1], p=2)
            n_pairs += 1
    if n_pairs == 0:
        return torch.tensor(0.0)
    avg_drift = total_drift / n_pairs
    return -DIVERSITY_WEIGHT * avg_drift  # Negative because we want to MAXIMIZE drift


def a2c_update(observer, navigator, opt, trajectories, advantages_list, returns_list,
               all_zs_rollout, clip=PPO_CLIP, div_weight=DIVERSITY_WEIGHT):
    """A2C+PPO update with diversity loss on CURRENT policy's latents (not rollout)."""
    (all_obs_grids, all_obs_pos, all_radars, all_actions,
    ...
    for _ in range(PPO_EPOCHS):
        # Forward pass: get current policy's latents (NOT from rollout cache)
        z_current, values = observer(grids_cat, pos_cat)
        ...
        # Maximize ||z_t - z_{t-1}|| across consecutive steps
        total_drift = 0.0
        n_pairs = 0
        offset = 0
        for ep_zs_rollout in all_zs_rollout:
            ...
            if ep_len < 2:
                offset += ep_len; continue
            # Get current policy's z for this episode's states
            z_ep = z_current[offset:offset + ep_len]
            for t in range(1, ep_len):
                total_drift += torch.norm(z_ep[t] - z_ep[t-1], p=2)
                n_pairs += 1
            offset += ep_len
        ...
        torch.nn.utils.clip_grad_norm_(
            list(observer.parameters()) + list(navigator.parameters()), 0.5)
        opt.step()
...
def evaluate(observer, navigator, num_eps=200):
    ...
    for _ in range(num_eps):
        ...
        for step in range(MAX_STEPS):
            ...
            radar = get_radar(grid, r, c)
            with torch.no_grad():
                z, _ = observer(grid.flatten().unsqueeze(0), pos.unsqueeze(0))
                action = navigator(radar.unsqueeze(0), z, deterministic=True).squeeze(0)
            new_pos = torch.clamp(pos + action, 0, GRID_SIZE - 1)
            r_new, c_new = int(round(new_pos[0].item())), int(round(new_pos[1].item()))
            r_new, c_new = max(0, min(GRID_SIZE-1, r_new)), max(0, min(GRID_SIZE-1, c_new))
            if grid[r_new, c_new] == 1.0:
                wall_hits += 1; break
            pos = new_pos
    ...
    for _ in range(num_eps):
        ...
        path = a_star_path(grid.numpy(), start, target)
        if path:
            greedy_success += 1; greedy_steps += len(path) - 1
        else:
...
# --- LATENT EVOLUTION TEST (unchanged) ---
...
def latent_evolution_test(observer, navigator, out_dir):
    ...
    plt.tight_layout()
    fname = out_dir / "maze_latent_evolution_v5.png"
...
def plot_summary(warmup_hist, ppo_history, eval_results, out_dir):
    ...
    ax.set_title("Phase 1: Warm-Start"); ax.set_xlabel("Epoch"); ax.set_ylabel("MSE"); ax.set_yscale("log"); ax.grid(True, alpha=0.3)
    ...
    ax.set_title("Phase 2: PPO+Diversity"); ax.set_xlabel("Eval #"); ax.legend(loc='upper left'); ax.grid(True, alpha=0.3)
    ax = axes[1, 0]; labels = ["Neuralese", "A*"]; successes = [eval_results["neur_success"], eval_results["greedy_success"]]
    steps = [eval_results["neur_steps"], eval_results["greedy_steps"]]
    x = np.arange(2); width = 0.35
    ax.bar(x - width/2, successes, width, label="Success Rate", color="blue", alpha=0.7)
    ax2 = ax.twinx(); ax2.bar(x + width/2, steps, width, label="Avg Steps", color="red", alpha=0.7)
    ax.set_xticks(x); ax.set_xticklabels(labels); ax.set_ylabel("Success Rate"); ax2.set_ylabel("Avg Steps")
    ax.set_title("Final Evaluation"); ax.set_ylim(0, 1.1); ax.grid(True, alpha=0.3, axis='y')
    ax = axes[1, 1]
    metrics = (f"PPO clip={PPO_CLIP}, λ={GAE_LAMBDA}\n"
               f"Diversity weight: {DIVERSITY_WEIGHT}\n"
               f"Critic DETACHED from shared trunk\n"
               f"Wall hits/ep: {eval_results['wall_hits_neur']:.2f}\n"
               f"Entropy: {ppo_history['entropy'][-1] if ppo_history['entropy'] else 'N/A':.3f}")
    ax.text(0.1, 0.5, metrics, fontfamily='monospace', fontsize=10, va='center', transform=ax.transAxes)
    ax.set_title("Config"); ax.axis('off')
    plt.suptitle("Neuralese v5 — PPO + Diversity Loss", fontsize=14, fontweight='bold')
    plt.tight_layout()
    fname = out_dir / "maze_results_v5.png"; fig.savefig(fname, dpi=150); plt.close(fig)
    print(f"  Saved: {fname}")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("NEURALESE v5 — PPO + Diversity Loss")
    print("=" * 60)
    print(f"  Diversity weight: {DIVERSITY_WEIGHT}→0.01 (decay=×{DIVERSITY_DECAY})")
    print(f"  Critic: detached from shared trunk")
    print(f"  LR: {RL_LR}")
    ...
    print("\n[Phase 2] PPO + Diversity Training (scheduled decay)...")
    opt = optim.Adam(list(observer.parameters()) + list(navigator.parameters()), lr=RL_LR)
    ppo_history = {"success": [], "wall_hits": [], "p_loss": [], "v_loss": [], "entropy": [], "div_loss": []}
    ep_counter = 0
    episodes_per_update = 32
    div_weight = DIVERSITY_WEIGHT

    while ep_counter < PPO_EPISODES:
        trajectories = collect_trajectories_diverse(observer, navigator, episodes_per_update)
        all_obs_grids, all_obs_pos, all_radars, all_actions, all_log_probs, \
            all_rewards, all_values, all_terminated, all_zs = trajectories

        advantages, returns = compute_gae(all_rewards, all_values, all_terminated,
                                          observer, all_obs_grids, all_obs_pos)

        p_loss, v_loss, ent, div = a2c_update(observer, navigator, opt, trajectories,
                                               advantages, returns, all_zs, PPO_CLIP,
                                               div_weight=div_weight)

        # Decay diversity weight
        div_weight = max(0.01, div_weight * DIVERSITY_DECAY)
        ...
        if ep_counter % 200 == 0 or ep_counter >= PPO_EPISODES:
            ...
            print(f"  PPO ep {ep_counter:5d}: succ={eval_r['neur_success']:.1%} "
                  f"walls={eval_r['wall_hits_neur']:.2f} p={p_loss:.4f} v={v_loss:.2f} "
                  f"ent={ent:.3f} div={div:.4f} div_w={div_weight:.3f}")
    ...
    print(f"  Neuralese: {eval_results['neur_success']:.1%} success, "
          f"{eval_results['neur_steps']:.1f} steps, {eval_results['wall_hits_neur']:.2f} walls/ep")
    print(f"  A*: {eval_results['greedy_success']:.1%} success, {eval_results['greedy_steps']:.1f} steps")
